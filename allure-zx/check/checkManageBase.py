...
class CheckResult():
    ...
    def getWorkOrderId(self,*args):
        """
        """
        Logger = Trace("CheckResult::getWorkOrderId")
        try:
            sql = "select workorder_id from workorder order by workorder_id desc limit 1"
            return self.checkdb.getField(sql,'hoolink_omc',*args)
        except Exception, e:
            errMsg = traceback.format_exc()
            Logger.error(self.logId,"error: %s" % errMsg)
            return None

    def __checkBillFieldByOrgBrn(self,brn,**kwargs):
        """
        """
        Logger = Trace("CheckResult::__checkBillFieldByOrgBrn")
        try:
            sql = 'select * from Bill where originalRecordNumber = %s'%brn
            return self.checkdb.checkField(sql,'bill',**kwargs)
        except Exception, e:
            errMsg = traceback.format_exc()
            Logger.error(self.logId,"error: %s" % errMsg)
            return None

    def __checkBillFieldByBn(self,bn,**kwargs):
        """
        """
        Logger = Trace("CheckResult::__checkBillFieldByBn")
        try:
            sql = "select * from Bill where businessNumber = '%s'"%bn
            return self.checkdb.checkField(sql,'bill',**kwargs)
        except Exception, e:
            errMsg = traceback.format_exc()
            Logger.error(self.logId,"error: %s" % errMsg)
    ...
    def check_removeUser(self,resp,userId,enabled):
        ...
        if self.__dealwithDeviceResp__(resp):
            ...
            return self.__checkByRecord(db='hoolink_manage',table='manage_user',criteria={"select":"enabled","where":"id=%s"%userId},
                                        enabled=enabled)
        return False

    @logging("CheckResult", "check_enableOrDisableUser")
    def check_enableOrDisableUser(self,resp,userId,user_status):
        """
        """
        if self.__dealwithDeviceResp__(resp):
            user_status = 1 if user_status is True else 0
            return self.__checkByRecord(db='hoolink_manage',table='manage_user',criteria={"select":"user_status","where":"id=%s"%userId},
        ...
        try:
            if self.__dealwithDeviceResp__(resp):
                ...
                return self.__checkByRecord(db='hoolink_base', table='base_area',
                                            ...
                                            enabled=enabled)
                    # for
                    # return self.__checkByRecord(db='hoolink_device', table='device_pole',
                    #                         criteria={"select": "area_desc,area_name,project_id,enabled",
                    #                                   "where": "area_id=%s" % resp.data.baseAreaWithBLOBs.areaId},
                    #                         area_desc=resp.data.baseAreaWithBLOBs.areaDesc,
                    #                         area_name=resp.data.baseAreaWithBLOBs.areaName,
    ...
    def check_createWorkOrder(self,resp,projectId,workorderContent,workorderFaultType,workorderLevel,workorderType,workorderAccendantId):
        ...
        try:
            if self.__dealwithDeviceResp__(resp):
                ...
                # return self.__checkByRecord(db='hoolink_omc', table='workorder',
                #                             criteria={"select": "*", "limit": "1",
                #                                       "order": "workorder_id desc"},
                #                             project_id=projectId, workorder_content=workorderContent,
                #                             workorder_fault_type=workorderFaultType, workorder_level=workorderLevel,
                #                             workorder_type=workorderType, workorder_accendant_id=workorderAccendantId)
            return False
        except Exception, e:
