...
class HttpSession(requests.Session):
    ...
    def request(self, method, url, **kwargs):
        ...
        """
        logging = Trace('HttpSession::request')
        # prepend url with hostname unless it's already an absolute URL
        url = self._build_url(url)
        logging.info(" Start to {method} {url}".format(method=method, url=url))
        # logging.debug(" kwargs: {kwargs}".format(kwargs=kwargs))
        # logging.debug(" kwargs: {kwargs}".format(kwargs=json.dumps(kwargs,ensure_ascii=False,indent=4).decode('utf8')))
        ...
        # if "HttpNtlmAuth" in kwargs:  what
        ...
        # response = json.dumps(request_meta,ensure_ascii=False,indent=4).decode('utf8')
        for key,value in request_meta.iteritems():
