...
from proc.hoolinkDevice import hoolinkDeviceTool
...
from proc.manageEdmTool import manageEdmTool
...
manageEdmTool = manageEdmTool(logId, env)
abilitytool = abilityTool(logId, env)
hoolinkDeviceTool = hoolinkDeviceTool(logId, env)
...
def check_resp_msg(resp, expectedMessage):
    with pytest.allure.step(
            "测试结果校验：{0} == {1}，{2} == {3}".format(resp.message, expectedMessage, resp.status, resp.checker)):
...
def gen_expire_time(minute):
    nowTime = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    expireTime = HumanDateTime(nowTime).add_minute(minute).strftime('%Y-%m-%d %H:%M:%S')
    return expireTime
...
if __name__ == '__main__':
    ...
    j = {"status": True, "data": {"conflictMap": {"createSceneName_16863117492": [{"deviceTimeStrategyVO": {
        "deviceType": "5", "onTimeStr": "19:52:00", "onTime": "1970-01-01 19:52:00", "weekTime": "1,2,3,4,5",
        "manageId": 7843, "timeType": 2, "strategyId": 8255, "offTime": "1970-01-01 19:54:00",
        "offTimeStr": "19:54:00"}, "dimmingMap": {}, "deviceLights": [
        {"creator": "test_zx1", "created": "2018-12-27T07:32:28.000Z", "externalId": 477, "lightStatus": 2,
         "subTypeId": 19, "enabled": True, "lightMacAddress": "123888", "updator": "fuxiuxia22", "lightName": "测试单灯1",
         "lightId": 324, "dimmingValue": 0, "lightNo": "2810", "updated": "2019-01-21T07:21:47.000Z"}]}],
                                                  "createSceneName_41487517755": [{"deviceTimeStrategyVO": {
                                                      "deviceType": "5", "onTimeStr": "19:53:00",
                                                      "onTime": "1970-01-01 19:53:00", "weekTime": "1,2,3,4,5",
                                                      "manageId": 9924, "timeType": 2, "strategyId": 10191,
                                                      "offTime": "1970-01-01 19:55:00", "offTimeStr": "19:55:00"},
                                                                                   "dimmingMap": {}, "deviceLights": [
                                                          {"creator": "test_zx1", "created": "2018-12-27T07:32:28.000Z",
                                                           "externalId": 477, "lightStatus": 2, "subTypeId": 19,
                                                           "enabled": True, "lightMacAddress": "123888",
                                                           "updator": "fuxiuxia22", "lightName": "测试单灯1",
                                                           "lightId": 324, "dimmingValue": 0, "lightNo": "2810",
                                                           "updated": "2019-01-21T07:21:47.000Z"}]}]}},
         "message": "场景管理读取成功", "msgMode": "PROMPT", "indexFlag": None, "errorCode": None,
         "txId": "7c1a0aacfea846bab1838cf7856f567e"}
